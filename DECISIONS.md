...
## 2026-09-17 — Ported the deferred-until-payment registered-agent email, the fulfillment-gate, Resend email transport, and the Worker-Deal-then-stage-update CRM lifecycle from a sibling branch (`feat/pdf-context-composer`) onto `main`

**Rationale:** a parallel branch (`feat/pdf-context-composer`) independently built the same Gate 2 feature area — checkout, Stripe webhook, PDF composer, registered-agent acceptance — against a migration history that was never actually applied to the live database (confirmed against `information_schema` and the live `schema_migrations` table: `main`'s own 0004-0011 are the ones reconstructed from what's really live). Rather than merge that branch's history (which would also pull in its stale, never-deployed migrations and an entirely different Stripe/PDF architecture), the four pieces of *functionality* it had that `main` didn't were ported directly onto `main`'s actual files:

1. **Resend email transport** (`registeredAgent/emailSender.ts`, ported near-verbatim) — same flagged exception to GOVERNANCE.md rule 5 as before. `RESEND_API_KEY` / `RESEND_FROM_EMAIL` added to `.env.example`.
2. **RA acceptance actually sends email now.** `registeredAgent/acceptanceEmail.ts` (new, shared module — deliberately not defined inside either route file, so `routes/registeredAgent.ts` and `routes/webhooksStripe.ts` don't import each other) closes the gap `routes/registeredAgent.ts`'s own comment named ("this does not send an email... this session cannot verify end-to-end without credentials"). `POST /api/registered-agent/request-acceptance` now sends immediately; the new `POST /registered-agent/select` (matching the already-shipped `florida-business-launchpad` frontend's contract exactly) persists the choice only and defers the send to payment confirmation.
3. **Fulfillment gate.** `routes/webhooksStripe.ts` no longer sets `orders.fulfillment_status = 'ready'` purely from `needsSunbizFiling` — a `registered_agent_path = 'customer'` order also needs `registered_agent_status = 'accepted'` first. `routes/registeredAgent.ts`'s accept endpoint (and the house-RA branch of `/registered-agent/select`) re-evaluates and flips any already-paid, now-unblocked order via `reevaluateFulfillmentAfterAcceptance`.
4. **Zoho Deal lifecycle, adopted per explicit instruction:** the Cloudflare Worker creates the Deal at intake; payment confirmation updates that Deal's *stage* rather than creating a second Deal. `routes/checkout.ts` reads `filing_sessions.crm_deal_id` and carries it as Stripe Checkout metadata (`stripe/restClient.ts`'s `createCheckoutSession` gained a `metadata` param); `routes/webhooksStripe.ts` reads it back from the metadata already present on the (signature-verified) webhook event body — no extra Stripe round-trip, matching this client's existing trust posture — and enqueues a new `crm_sync_queue` `sync_type = 'deal_stage_update'` job; `zoho/client.ts` gained `updateDealStage` (`PUT /crm/v2/Deals/{id}`, never create) to run it. **The existing `paid_deal`-creation path (`syncOrderEvent`) is intentionally left in place as the fallback** for whenever `crm_deal_id` isn't yet known at payment time — so an order is never left with zero Deal linkage while the Worker-to-session capture rolls out. `ZOHO_DEAL_STAGE_ON_PAYMENT` is left unset in `.env.example`, same reasoning as `feat/pdf-context-composer`'s own entry: no confirmed "this means paid" Stage value exists yet in this org's live picklist, and a guessed one would only enqueue a job that dead-letters.

**A new, real gap this port had to close to make any of the above possible:** `filing_data` was never in `routes/session.ts`'s `SESSION_FIELDS` whitelist — nothing on `main` could ever write it, even though `pdf/context.ts` and `fulfillment/fulfillmentWorker.ts` already read it. Added back; it carries none of the P0 fix's forgery risk (a client can only affect its own filing content this way, never another customer's payment or CRM record).

**A security gap surfaced, not fully closed:** `crm_deal_id` was deliberately excluded from the P0-fixed `SESSION_FIELDS` list — the same forgery-risk class as the `payment_status` hole that fix closed applies here too (a client could point its own session at an unrelated real Zoho Deal id). Rather than reopen that whitelist, a narrow, purpose-built path was added instead: `POST /api/session/stage` accepts `crm_deal_id` outside `SESSION_FIELDS`, written via `COALESCE(crm_deal_id, $new)` — settable exactly once per session, never overwritable afterward. This does not verify the id is real or belongs to this session's Contact (that would need a Zoho round-trip this request path doesn't make); a forged-but-plausible id still reaches `updateDealStage`, which fails harmlessly if it doesn't exist. Logged as a known residual gap, not fixed here.

**Also preserved as-is, not silently "fixed" while porting through the same area:** `routes/registeredAgent.ts`'s acceptance token remains an unhashed, directly-stored random value (`randomBytes(24).toString("base64url")`) — `feat/pdf-context-composer`'s equivalent hashed it. Changing `main`'s token scheme was outside what this port was asked to do.

**Files:** `registeredAgent/emailSender.ts` (new), `registeredAgent/acceptanceEmail.ts` (new), `offers/sunbizFiling.ts` (new — `needsSunbizFiling` factored out of `routes/webhooksStripe.ts` so `routes/registeredAgent.ts` can reuse it without a route-to-route import), `routes/registeredAgent.ts`, `routes/webhooksStripe.ts`, `routes/session.ts`, `routes/checkout.ts`, `stripe/restClient.ts`, `zoho/client.ts`, `sync/worker.ts`, `.env.example`, `package.json` (new dependency: `resend`). No migration — reuses `filing_sessions.crm_deal_id`/`filing_data` (0001/0004), `orders.crm_deal_id` (0007), and `crm_sync_queue.order_id` (0013) as they already exist live. `main`'s own unique Gate 2 work (Offer Master/`offers` table, `GET /api/offers`, the `cancel_url` fix, the deploy runbook) is untouched.

**Supersedes:** —

## 2026-09-17 — crm_deal_id ownership verification added to updateDealStage — mitigates, does not resolve, the trust boundary flagged 2026-09-17

**Rationale:** the prior entry (same date, "Ported the deferred-until-payment...") flagged that `crm_deal_id` capture at `POST /api/session/stage` is unauthenticated: a client can claim any string as its Deal id, once, via the `COALESCE`-guarded set-once path. This entry closes the actual attack this enables — a payment-triggered stage WRITE landing on a Deal the paying customer doesn't own — without touching that capture path at all.

`zoho/client.ts`'s `updateDealStage` now verifies ownership before ever writing: it fetches the Deal, follows its `Contact_Name` lookup to that Contact's `Email`, and requires it to match `expected_email` (`filing_sessions.email`, captured fresh at payment-confirmation time by `routes/webhooksStripe.ts` and carried in the queued job's own payload — never re-trusted from anywhere else). A mismatch, a Deal with no linked Contact, or a Deal that doesn't exist at all are `permanent` failures (`ZohoSyncResult.permanent`) — `sync/worker.ts`'s `recordOutcome` now sends those straight to `dead_letter` instead of the normal 1m-24h backoff schedule, since none of them resolve by retrying. The failure reason is preserved in `crm_sync_queue.last_error` — a durable, queryable record, not a transient log line.

**Explicitly NOT resolved, per direct instruction not to consider this closed:** the capture path itself (`routes/session.ts`) still accepts any client-supplied string as a session's `crm_deal_id`, once, with no verification at write time. What this entry changes is the *consequence* of that: exploiting it to affect a real Deal now requires the attacker to also know (or control an email address matching) that Deal's actual linked Contact's email — real PII belonging to the account holder, not something guessable from the Deal id alone. That is a materially higher bar than before, but it is a mitigation of the blast radius, not a fix of the underlying trust boundary. The capture path itself remains open. A real fix would need either the Cloudflare Worker to call the data-spine directly and authenticate itself (rather than the browser relaying an unauthenticated value), or a signed/short-lived token the Worker mints and the frontend merely forwards — neither is built.

**Files:** `zoho/client.ts` (`updateDealStage`, `DealStageUpdatePayload.expected_email`, `ZohoSyncResult.permanent`), `routes/webhooksStripe.ts` (passes `expected_email` when enqueueing; skips enqueueing entirely — rather than guaranteeing a dead-lettered job — when the filing_session has no email to verify against), `sync/worker.ts` (`recordOutcome`'s `permanent`-failure fast path). No migration.

**Supersedes:** —

---

## 2026-09-18 — Pre-payment PDF generation added (Requirement 1 of the 2026-09-18 requirement handoff); a pre-existing filing_data shape mismatch that silently blocked ALL PDF generation, pre- or post-payment, was found and fixed in the same pass

**Rationale:** the business approved three new non-negotiable requirements for the funnel (PDF evidence before payment, a Zoho Books invoice at package selection, a Zoho Books payment on confirmed Stripe payment). This entry covers only Requirement 1's narrowest slice — an actual PDF reachable from a live intake submission today — not Requirements 2/3, which remain unstarted.

**A more fundamental, previously-undiscovered bug was found while implementing this**, not something this entry introduces: `florida-business-launchpad/src/lib/canonicalFilingData.ts` (the frontend's ONLY mapping from `IntakeData` into `filing_sessions.filing_data`) never actually produced `FilingSessionRecord`'s shape, despite its own comment claiming the field names were "deliberately identical." It sent `principal_street/city/state/zip` instead of a combined `principal_address`; `agent_choice`/`agent_street` etc. instead of `registered_agent_path`/`registered_agent_florida_address`; and `members`/`managers`/`additional_authorized_persons` as separate arrays instead of one flat `authorized_persons` array. `buildPdfContext()`'s completeness check requires exactly those combined field names — every one of them was always absent, so `buildPdfContext` would have reported EVERY filing as incomplete regardless of what the customer entered. This was not specific to pre-payment generation: it would have silently parked every already-paid order's fulfillment at `requires_review` with a misleading "filing data incomplete" error, forever, never actually rendering a PDF for a real paying customer. Found by tracing the actual data flow end-to-end (frontend mapping -> `filing_sessions.filing_data` -> `buildPdfContext`), not assumed from either side's code in isolation. Fixed by rewriting `buildCanonicalFilingData` to actually combine these fields into `FilingSessionRecord`'s real shape (address formatting, `agentChoice === "damian" -> "house"`, `members`/`managers` selected by `managementStructure` plus `additionalAuthorizedPersons` overflow, filtering empty placeholder rows, and normalizing `"Immediately"` to `"Immediate"`).

**Requirement 1 slice implemented:** new `POST /api/filing-session/:filingSessionId/pdf` (`routes/filingDocument.ts`) generates the Articles of Organization PDF directly from a filing session's already-persisted `filing_data`, independent of payment/order state — no human-approval gate, matching the requirement's explicit instruction that this is a visibility/evidence control only. Idempotent per session: a pre-payment document (`filing_documents.order_id IS NULL`) already on record is returned as-is rather than regenerated, enforced by a partial unique index (migration `0015_filing_documents_pre_payment`), not just application logic. Reuses the existing `buildPdfContext`/`generateArticlesOfOrganizationPdf`/`filing_documents` machinery the post-payment fulfillment worker already used — no new PDF rendering path, no new external dependency, per the explicit "use existing capabilities" governance rule in the requirement handoff.

**NOT done in this pass, and Gate 2 stays open on this requirement until they are:** internal-reviewer delivery (the PDF is generated and downloadable by the customer's own browser today; nothing yet emails or otherwise delivers a copy to a designated internal reviewer); Requirements 2 and 3 (Zoho Books invoice/payment) are entirely unstarted; the `annual_report_due_date` field `buildPdfContext` requires for Future/Backdated effective dates is still not collected or computed anywhere in the frontend, so that specific path still correctly surfaces as an incomplete-data gap rather than succeeding.

**Migration 0015 was written but could not be applied from this session** — it was blocked by this environment's own safety controls on modifying shared/production resources (schema changes to the live Supabase database require the human operator's direct action or explicit run-it-now confirmation, not an agent doing it unilaterally). The migration file exists at `server/migrations/0015_filing_documents_pre_payment.sql`; until it is applied, the new route's INSERT will fail against the still-`NOT NULL` `order_id` column, so this capability is implemented and tested but NOT yet live.
